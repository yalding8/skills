...
- **进入终面的候选人**（独立文件夹）：`~/hire/interviews/YYYY-MM-DD-姓名-岗位简称/`
  - `resume.pdf`（简历副本）
  - `record.md`（4 部分：前序评价/简历匹配/5 题答案/最终评价）
- **未进入终面的候选人**（月度汇总）：`~/hire/interviews/rejected/YYYY-MM.md`
- 模板：
  - `~/hire/templates/position-template.md`（新岗位画像）
  - `~/hire/templates/interview-template.md`（独立文件夹的 record.md 骨架）
  - `~/hire/templates/rejected-template.md`（月度汇总骨架）
...
**前序面试评价（可选输入）**：
- 用户可能会**同时**或**后续补充**提供 HR 沟通 / 主管沟通 / 其他面试官的反馈
- 这些信息**会改变你的评估**（如：主管认为业绩可信、HR 提供了候选人的稳定性信号、薪资期望、家庭情况等）
- 接收后必须：
  1. 明确告诉用户**哪些原判断被推翻 / 加强 / 新增**
  2. 在归档中记录前序评价的关键内容
  3. 如果是在 Step 2 之后才补充，重新输出更新版评估
...
**轨道 A：进入终面的候选人 → 独立文件夹**
- 文件夹：`~/hire/interviews/YYYY-MM-DD-姓名-岗位简称/`
- 内容：
  1. `resume.pdf` — 用 `cp` 拷贝原始简历（若是 PDF；其他格式保留原扩展名）
  2. `record.md` — 基于 `~/hire/templates/interview-template.md` 骨架写入完整面试记录
- 时机：**用户在 Step 2 确认"进入终面"时立即创建文件夹并拷贝简历**；Step 3-5 的产出增量写入 `record.md`
- 完成后告诉用户文件夹路径

**轨道 B：未进入终面的候选人 → 月度汇总**
- 文件：`~/hire/interviews/rejected/YYYY-MM.md`（若不存在则用 `rejected-template.md` 创建）
- 内容：在该月文件底部追加一条记录，包含姓名、岗位、简历路径、Claude 风险点、是否推翻等
- 时机：**用户在 Step 2 确认"不进入终面"时写入，流程结束**

**命名规范**：
- 姓名用中文（macOS 文件名支持）
- 岗位简称如：`缴费销售经理` / `运营经理` / `缴费顾问` / `留学渠道BD`
- 示例：`2026-04-15-孙思涵-缴费销售经理/`

**特殊情况**：如果用户推翻 Claude 的"不进入终面"建议，执行**轨道 A**（独立文件夹），同时在 `rejected/YYYY-MM.md` 附一条"Claude 建议拒但用户坚持面"的指针记录（便于日后复盘）
